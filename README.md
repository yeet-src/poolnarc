...
- **Connections established before the scan started, which are invisible for their entire lifetime.** Per-connection state is created only on the `TCP_ESTABLISHED` transition, and the byte hooks return early when a socket has no entry, so a connection that was already open when the probe attached is never tracked no matter how much traffic it moves. A miner that connected an hour ago and is still earning produces a `CLEAN` verdict. Start the scan first, and treat a clean result as a statement about connections opened during the window rather than about the host.
...
It means no connection *opened during that window* matched the two rules. That is a narrower claim than it sounds: a miner whose connection was already established when the scan started is never tracked at all, so a long-running miner reports `CLEAN` no matter how long you scan. Lengthening `--duration` helps with miners that connect on an interval; it does nothing for one that is already connected. Restarting the suspect process, or scanning on a schedule so you catch the next reconnect, is what closes that gap. [What it can't see](#what-it-cant-see) has the rest.
